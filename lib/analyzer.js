...
function buildScore(ohlcv, ticker) {
  ...
  if (adxVal !== null) {
    if (adxVal < CONFIG.ADX_RANGE_THRESHOLD) score += 20;
...
export async function analyzeMarket(ohlcvMap, symbols) {
  const tickers = await fetchTickers(symbols);
  const results = [];
  for (const s of symbols) {
    const o = ohlcvMap[s] || [];
    const t = tickers[s] || null;
    const r = buildScore(o, t);
    if (r) results.push(r);
  }
