...
export async function getCycles(limit = 15) {
  const sb = getSupabase();
  return sb.from('cycles').select('*').order('created_at', { ascending: false }).limit(limit);
}

export async function getSignals(limit = 15) {
  const sb = getSupabase();
  return sb.from('signals').select('*').order('created_at', { ascending: false }).limit(limit);
}

export async function getOrders(limit = 15) {
  const sb = getSupabase();
  return sb.from('orders').select('*').order('created_at', { ascending: false }).limit(limit);
}