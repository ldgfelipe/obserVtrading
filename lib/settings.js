...
export const DEFAULT_SETTINGS = {
  ...
  INVEST_MODE: 'percent', // 'percent' | 'fixed'
  INVEST_PERCENT: 10, // % del balance por trade si INVEST_MODE=percent
  MAX_AMOUNT_PER_TRADE_MXN: 50, // monto fijo si INVEST_MODE=fixed
  GRID_RANGE_PCT: 5, // rango total del grid (porcentaje alrededor del precio)
  GRID_LEVELS: 6, // numero de niveles de compra/venta del grid
...
function normalize(raw) {
  ...
  return {
    ...
    STRATEGY: ['momentum', 'grid'].includes(base.STRATEGY) ? base.STRATEGY : 'momentum',
    INVEST_MODE: ['percent', 'fixed'].includes(base.INVEST_MODE) ? base.INVEST_MODE : 'percent',
    INVEST_PERCENT: toNum(base.INVEST_PERCENT, 10),
    MAX_AMOUNT_PER_TRADE_MXN: toNum(base.MAX_AMOUNT_PER_TRADE_MXN, 50),
    GRID_RANGE_PCT: toNum(base.GRID_RANGE_PCT, 5),
    GRID_LEVELS: Math.max(2, Math.min(20, Math.round(toNum(base.GRID_LEVELS, 6)))),
